...
Season.prototype.debug = function (id,msg) {
  if (!id || id == this.id) {
    console.log("{ Id: " + this.id + " Name: " + this.name + " Start: " +date_as_ymd(this.start) + " End: " + date_as_ymd(this.end) + " }",msg);
  }
}
...
Season.splice = function (season1,season2) {
  ...
  if ( season1.id == 6 || season2.id == 6) { console.log("Created Spliced Seasons", season_array); }
  return season_array;
}
...
//     console.log("Descending into array for intersections");
    var overlap = false;
    for (var j = 0; j < new_seasons.length; j++) {
//       console.log("j is: ",j);
      var s2 = new_seasons[j];
      if (s2.id == s1.id) {
        continue;
      } else if (s1 && s2 && s1.intersects_with(s2)) {
...
//       console.log("overlap was found");
    }
 // This was the old way, but doesn't work, because seasons that intersect are not always right next to each other, but
// there can be multiple sub seasons, so we need to split those as well.
